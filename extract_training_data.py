...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        # array is of the format [stack1, stack2, stack3, buf1, buf2, buf3]

        # special symbols
        special_symbols = ["CD", "NNP", "UNK", "ROOT", "NULL"]

        # get top three words on the stack
        # pad with "<NULL>"
        ...
        i = -1
        j = 0
        while j < len(state.stack) and i >= -3:
            stack.append(state.stack[i])
            j += 1
            i -= 1
        ...
        i = -1
        j = 0
        # get top three words on the buffer
        while j < len(state.buffer) and i >= -3:
            buf.append(state.buffer[i])
            j += 1
            i -= 1
        ...
        for i in range(len(lst)):
            word_id = lst[i]
            if word_id == "<NULL>":
                lst[i] = self.word_vocab["<NULL>"]
            else:
                part_of_speech = pos[word_id]
                word = words[word_id]
                if part_of_speech is None:
                    lst[i] = self.word_vocab["<ROOT>"]
                else:
                    word = word.lower()
                    if part_of_speech in special_symbols:
                        symbol = "<" + part_of_speech + ">"
                        lst[i] = self.word_vocab[symbol]
                    else:
                        if word not in self.word_vocab:
                            lst[i] = self.word_vocab["<UNK>"]
                        else:
                            lst[i] = self.word_vocab[word]
    ...
    def get_output_representation(self, output_pair):
        ...
        # (left-arc, 45 labels)
        # (right-arc, 45 labels)
        # total - 91 possible pairs

        rep = np.zeros(91)

        # get index of output pair in self.output_labels dictionary
        if output_pair in self.output_labels:
            idx = self.output_labels[output_pair]

            # set one-hot value at idx to 1
            rep[idx] = 1
